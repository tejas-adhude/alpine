...
class myKite:
    ...
    def get_historical_data(self,nowTime,timeFrame,scName=None,tokenId=None,noCandle=None,toDateTime=None,fromDateTime=None):
        ...
        if toDateTime:
            ...
            toDateTime=toDateTime.replace(second=0,microsecond=0)
        ...
        if fromDateTime:
            ...
            fromDateTime=fromDateTime.replace(second=0,microsecond=0)

        if noCandle:noCandle=int(noCandle)

        #calculating the fromDateTime when toDateTime and noCandle given
        if (noCandle and toDateTime and not fromDateTime): 
            fromDateTime=toDateTime - datetime.timedelta(minutes=noCandle*cmul)
